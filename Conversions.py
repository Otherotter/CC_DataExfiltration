...
def binary_converter(message):
    ...
    if msg_list[0] == 'ALL':
        ...
        res = echo
    elif msg_list[0] == 'DROPPED':
        res = dropped
    elif msg_list[0] == 'X-SUCCESSFUL':
        res = x_successful
    elif msg_list[0] == "CLIENT-COMMUNICATING":
        res = comm
    else:
        res = ip
    ...
    #     command = "SEND"
        ...
        if message[124:128] == "0000":
            ...
            while index1 < len(message):
                ...
                index2 += 8
            return command + ' ' + new_message
        if message[124:128] == "0001":
            command += " "
            command += "DISCONNECT"
            return command
        elif message[124:128] == "0011":
            command += " "
            command += "ECHO"
            return command
        else:
            return command
    elif message[0:4] == '1000':
        command = "DROPPED"
        return command
    elif message[0:4] == '1001':
        command = "X-SUCCESSFUL"
        return command
    elif message[0:4] == '1010':
        command = "CLIENT-COMMUNICATING"
        return command
    else:
